...
// EX) 사용자가 버튼을 클릭, 웹페이지가 로드, input 필드에 입력하는 것 등
...
// addEventListner 메서드는 HTML 요소(객체)에 사용

const btnsButton = document.querySelectorAll('.btnsChange');

// addEventListener()는 인자로 이벤트와 롤백함수를 받음

// 이벤트: 이벤트의 종류를 문자열로 전달
// 롤백함수: 요소에 연결할 함수(이벤트 발생 시 실행 될 함수)
