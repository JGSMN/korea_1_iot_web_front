...
let fruits = ['banana', 'orange', 'mango'] // orange
...
// cf) 배열 메서드 & 롤백 함수(함수에 인자로 전달하는 함수)
// - forEach: 반환 X / 전체 요소에 동일한 기능을 적용

// - map: 새로운 배열을 반환 / 전체 요소에 동일한 기능을 적용

// - filter: 새로운 배열을 변환 / 해당 조건식을 참으로 통과하는 요소만 새로운 배열로 반환

// +) 대문자 변환: 문자열.toUpperCase()

// 배열 메서드에서 쓰이는 롤백함수의 인자 (value, index, array)
let upperCaseFruits = fruits.map(function(value, index, array) { 
  return value.toUpperCase()

});
// cf) 배열메서드의 롤백함수는 주로 화살표 함수 형태
fruits.map(()=> value.upperCase());
...
console.log(person); // { name: '이기석' , age: 50 isStudent: false }
